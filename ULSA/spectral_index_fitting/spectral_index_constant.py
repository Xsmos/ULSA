...
class constant_index(object):
    ...
    def read_file(self, downgrade_to=256, resolution=5):
        ...
        hpmap_408 = hp.read_map(
            self.file_dir + '/Haslam/haslam408_dsds_Remazeilles2014.fits')
        ...
        #####
        # the data from LWA
        hpmap_35 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-35.fits')
        hpmap_35 = self.minus_free_free(hpmap_35, 35, downgrade_to)
        hpmap_38 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-38.fits')
        hpmap_38 = self.minus_free_free(hpmap_38, 38, downgrade_to)
        hpmap_40 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-40.fits')
        hpmap_40 = self.minus_free_free(hpmap_40, 40, downgrade_to)
        hpmap_45 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-45.fits')
        hpmap_45 = self.minus_free_free(hpmap_45, 45, downgrade_to)
        hpmap_50 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-50.fits')
        hpmap_50 = self.minus_free_free(hpmap_50, 50, downgrade_to)
        hpmap_60 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-60.fits')
        hpmap_60 = self.minus_free_free(hpmap_60, 60, downgrade_to)
        hpmap_70 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-70.fits')
        hpmap_70 = self.minus_free_free(hpmap_70, 70, downgrade_to)
        hpmap_74 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-74.fits')
        hpmap_74 = self.minus_free_free(hpmap_74, 74, downgrade_to)
        hpmap_80 = hp.read_map(
            self.file_dir + '/LWA/healpix-all-sky-rav-rsclean-map-80.fits')
    ...
    def smoothing_data(self, downgrade_to):
        ...
        Dict = {'hpmap_408': hpmap_408, 'hpmap_45_old': hpmap_45_old, 'hpmap_35': hpmap_35, 'hpmap_38': hpmap_38, 'hpmap_40': hpmap_40,
                'hpmap_45': hpmap_45, 'hpmap_50': hpmap_50, 'hpmap_60': hpmap_60, 'hpmap_70': hpmap_70, 'hpmap_74': hpmap_74, 'hpmap_80': hpmap_80}
        for key, X in Dict.items():
            ...
            if key == 'hpmap_35':
                ...
                Mask_missing_region_lwa = hp.ud_grade(
                    Mask_missing_region_lwa, downgrade_to)
    ...
    def calculate_index(self, downgrade_to):
        ...
        hpmap_408, hpmap_45_old, hpmap_35, hpmap_38, hpmap_40, hpmap_45, hpmap_50, hpmap_60, hpmap_70, hpmap_74, hpmap_80 = Dict['hpmap_408'], Dict['hpmap_45_old'], Dict[
            'hpmap_35'], Dict['hpmap_38'], Dict['hpmap_40'], Dict['hpmap_45'], Dict['hpmap_50'], Dict['hpmap_60'], Dict['hpmap_70'], Dict['hpmap_74'], Dict['hpmap_80']
        ...
        for i in range(12*downgrade_to**2):
            mask_condition = np.array([hpmap_45_old[i]-2.725-self.I_E(45), hpmap_35[i]-2.725-self.I_E(35), hpmap_38[i]-2.725-self.I_E(38), hpmap_40[i]-2.725-self.I_E(
                40), hpmap_50[i]-2.725-self.I_E(50), hpmap_60[i]-2.725-self.I_E(60), hpmap_70[i]-2.725-self.I_E(70), hpmap_74[i]-2.725-self.I_E(74), hpmap_80[i]-2.725-self.I_E(80)])
            if self.v_file_dir != None:
                result_ = []
            ...
            value_freq = np.array([hpmap_45_old[i] - 2.725, hpmap_35[i]-2.725, hpmap_38[i]-2.725, hpmap_40[i]-2.725,
                                  hpmap_50[i]-2.725, hpmap_60[i]-2.725, hpmap_70[i]-2.725, hpmap_74[i]-2.725, hpmap_80[i]-2.725])
            if self.v_file_dir != None:
                append_value_freq = []
                for j in range(len(Value)):
